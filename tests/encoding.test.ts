...
describe('encoding', function () {
  describe('public key encoding', () => {
    test('returns correct encoding type', function () {
      ...
      expect(pubkey_encoded.type).toEqual('tendermint/PubKeySecp256k1');
    });

    test('throws error if public key of incorrect size is passed', function () {
      const pubkey = new Uint8Array([
        0x02, 0x75, 0x9f, 0x18, 0x2b, 0x3e, 0x3d, 0x29, 0x5c, 0x5d, 0x3b, 0x0d, 0x80, 0x5e, 0x8b, 0x1a, 0x4f, 0x13,
    ...
    test('throws error if public key of with incorrect starting bytes is passed', function () {
      const pubkey = new Uint8Array([
  ...
  describe('signature encoding', () => {
    test('encodeSecp256k1Signature', function () {
